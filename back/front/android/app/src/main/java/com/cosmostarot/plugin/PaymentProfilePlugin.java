...
public class PaymentProfilePlugin extends Plugin implements PurchasesUpdatedListener {
    ...
    public void purchaseItem(final PluginCall call) {
        ...
        if (billingClient == null || !billingClient.isReady()) {
            call.reject("결제 시스템이 아직 준비되지 않았습니다. initializePayment를 먼저 호출해주세요.");
            return;
        }
        ...
        billingClient.queryProductDetailsAsync(queryParams, new ProductDetailsResponseListener() {
            ...
            public void onProductDetailsResponse(BillingResult billingResult, QueryProductDetailsResult productDetailsResult) {
                List<ProductDetails> productDetailsList =
                        (productDetailsResult == null) ? null : productDetailsResult.getProductDetailsList();
    ...
    public void onPurchasesUpdated(BillingResult billingResult, List<Purchase> purchases) {
        ...
        if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK && purchases != null) {
            ...
            if (ids != null) {
                obfuscatedAccountId = ids.getObfuscatedAccountId();
            }
            purchaseInfo.put("accountId", obfuscatedAccountId);
