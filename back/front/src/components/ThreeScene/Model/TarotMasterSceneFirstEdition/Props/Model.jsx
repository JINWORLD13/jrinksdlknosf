...
import { useCrystalActionRefactored } from '../../../../../hooks/threeScene/useCrystalActionRefactored';
import { useCandleActionRefactored } from '../../../../../hooks/threeScene/useCandleActionRefactored';
import {
...
export default memo(function Model({
  ...
  onOpenReadingTypeChoice, // 수정 구슬 클릭 시 맞춤/제너럴 선택 창 먼저 열기
  ...props
}) {
  const {
    answerForm,
  ...
  // console.log('Actions in Model.jsx: ', actions);
...
반면, 두 번째 코드에서는 nodes와 materials를 수동으로 관리하므로, useGLTF의 잠재적 정리 메커니즘을 우회하게 됩니다. 이는 개발자가 직접 리소스 정리를 책임져야 함을 의미합니다. */
  ...
  const openSpreadModal = async e => {
    ...
    if (answerForm?.isWaiting === false) {
      const canOpenReadingTypeChoice =
        typeof onOpenReadingTypeChoice === 'function';
      // 호환성: 선택 모달을 못 여는 환경(구버전)에서는 기존처럼 로그인 체크 후 스프레드 모달로 진입
      if (!canOpenReadingTypeChoice) {
        const hasToken = isNative
          ? await hasAccessTokenForPreference()
          : hasAccessToken();
        if (!hasToken) {
      ...
      //! 안정 기능을 위해
      setWatchedAd(false);
      // 모달 표시: 수정 구슬 클릭 시 맞춤/제너럴 선택 창 먼저, 없으면 스프레드 모달 직접
